...
void SetDetailTexture(inout float4 color, Texture2D t, float2 uv, float depth, SamplerState samp)
{
            [branch]
    if (depth < 0.995f)
    {
        ...
        detail = t.Sample(LinearSampler, uv);
        
        float detailBrightness = 1.2f;
        
        color = color * detail * detailBrightness;
...
void SetDetailTexture(inout float4 color, Texture2D t, float2 uv, float depth)
{
    SetDetailTexture(color, t, uv, depth, LinearSampler);
}
...
void NormalMapping(float2 uv, Texture2D t, float3 normal, float3 tangent, SamplerState samp)
{
    MakeMaterial();
    float4 map = t.Sample(samp, uv);
    ...
    //ÅºÁ¨Æ® °ø°£
    float3 N = normalize(normal); //Z
    float3 T = normalize(tangent - dot(tangent, normal) * normal); //X
    float3 B = cross(normal, T); //Y
    float3x3 TBN = float3x3(T, B, normal);

    //ÀÌ¹ÌÁö·ÎºÎÅÍ ³ë¸Öº¤ÅÍ °¡Á®¿À±â
    float3 coord = map.rgb * 2.0f - 1.0f;

    //ÅºÁ¨Æ® °ø°£À¸·Î º¯È¯
    coord = mul(coord, TBN);

    float NDotL = dot(coord, -GlobalLight.Direction);

    Material.Diffuse *= NDotL;
}

void NormalMapping(float2 uv, Texture2D t, float3 normal, float3 tangent)
{
    NormalMapping(uv, t, normal, tangent, LinearSampler);
}

void SetNormalMapping(inout float4 color, float2 uv, Texture2D t, float3 normal, float3 tangent, SamplerState samp)
{
    float4 map = t.Sample(samp, uv);
...
void SetNormalMapping(inout float4 color,float2 uv, Texture2D t, float3 normal, float3 tangent)
{
    SetNormalMapping(color,uv, t, normal, tangent, LinearSampler);
}
...
float4 PS_Shadow(float4 position, float4 color)
{
    ...
    else if (ShadowQuality == 2)
    {
        ...
        float2 size = 1.0f / (ShadowMapSize * 1);
        float2 offsets[] =
        {
