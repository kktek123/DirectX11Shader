...
VertexOutput_Water VS_Water(VertexTexture input)
{
    ...
    //    float cosTime = cos(WaveTranslation + input.Uv.x) * WaveScale; //< 시간에 따른 코사인값

//﻿
    //float cosTime = cos(WaveTranslation + input.Uv.y * 10) * WaveScale;
    ...
    output.Uv += float2(WaveTranslation, 0);
    output.Uv2 += float2(WaveTranslation, 0);
    return output;
}
