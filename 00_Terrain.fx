...
void SetTerrainWorld(inout matrix world, VertexTextureNormalTangent input)
{
    world = input.Transform;
}
float4 Planes[6];
VertexTerrain VS_Terrain_Projector(VertexTextureNormalTangent input)
{
    VertexTerrain output;
    //SetTerrainWorld(World, input);
    VS_GENERATE

    output.Color = input.Color;
    output.detailUv = output.Uv * 100 - 10 * (int) output.Uv;
    //output.Clip = dot(WorldPosition(input.Position), ClipPlane);
    VS_Projector(output.wvpPosition_Sub, input.Position);
    return output;
}

VertexTerrain VS_PreRender_Reflection_Terrain(VertexTextureNormalTangent input)
{
    VertexTerrain output = VS_Terrain_Projector(input);

    matrix reflection = mul(World, mul(Reflection, Projection));
    output.Position = mul(input.Position, reflection);
...
DepthOutput VS_Depth_Terrain(VertexTextureNormalTangent input)
{
    DepthOutput output;
...
float4 GetLayerColor(SamplerState samp, float2 uv)
{
    ...
    //float4 layer1 = LayerMap1.Sample(LinearSampler, uv);
    //float4 layer2 = LayerMap2.Sample(LinearSampler, uv);

    //float alpha = AlphaMap.Sample(LinearSampler, uv).r - LayerColor1.r;
    
    //base = lerp(base, layer, (1 - alpha) * 0.4f);

    //alpha = AlphaMap.Sample(LinearSampler, uv).r - LayerColor2.r;
    //base = lerp(base, layer1, (1 - alpha) * 0.4f);
    
    //alpha = AlphaMap.Sample(LinearSampler, uv).r - LayerColor3.r;
    //base = lerp(base, layer2, (1 - alpha) * 0.4f);
...
float4 GetLayerColor(float2 uv, float4 color)
{
    float4 base = BaseMap.Sample(LinearSampler, uv);
    float4 layer = LayerMap.Sample(LinearSampler, uv);
    float4 layer1 = LayerMap1.Sample(LinearSampler, uv);
    ...
    float alpha = AlphaMap.Sample(LinearSampler, uv).r - color.r;
    
    base = lerp(base, layer, (1 - alpha) * 0.4f);

    alpha = AlphaMap.Sample(LinearSampler, uv).r - color.g;
    base = lerp(base, layer1, (1 - alpha) * 0.4f);
    
    alpha = AlphaMap.Sample(LinearSampler, uv).r - color.b;
    base = lerp(base, layer2, (1 - alpha) * 0.4f);
...
DomainOutput_TerrainLod DS_TerrainLod(CHullOutput_TerrainLod input, OutputPatch<HullOutput_TerrainLod, 4> patch, float2 uv : SV_DomainLocation)
{
    ...
    float3 position = lerp(p0, p1, uv.y); //TS가 완료된 정점의 위치
