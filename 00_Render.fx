...
float4 ClipPlane = float4(0, 0, 0, 0);
float4 CullPlane[4];
...
output.Uv = input.Uv; \
\
output.Culling.x = mul(float4(output.wPosition, 1), CullPlane[0]); \
output.Culling.y = mul(float4(output.wPosition, 1), CullPlane[1]); \
output.Culling.z = mul(float4(output.wPosition, 1), CullPlane[2]); \
output.Culling.w = mul(float4(output.wPosition, 1), CullPlane[3]); \
\
output.Clipping = mul(float4(output.wPosition, 1), ClipPlane); 
...
output.Position = ViewProjection(output.Position); \
output.wvpPosition = output.Position; \
output.wvpPosition_Light = output.Position; \
\
output.sPosition = WorldPosition(position); \
output.sPosition = mul(output.sPosition, ShadowView); \
output.sPosition = mul(output.sPosition, ShadowProjection);\
\
output.Clip = dot(WorldPosition(position), ClipPlane);
